...
class ENVIRONMENT : public RaisimGymEnv {
  public:
    explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
      ...
      ball_gc_init_.setZero(7); ball_gc_init_[1] = 10; ball_gc_init_[3] = 1; // put away the ball for now
      ...
      ball_gc_ref_.setZero(7); ball_gc_init_[1] = 10; ball_gc_init_[3] = 1; // put away the ball for now
  ...
  void reset() final {
    ...
    ball_->setState(ball_gc_ref_, ball_gv_ref_);
    updateObservation();
  }
  ...
  void updateObservation() {
    ...
    for(size_t bodyIdx = 1; bodyIdx < 13; bodyIdx++){
      ...
      if (bodyIdx == 4 || bodyIdx == 6 || bodyIdx == 8 || bodyIdx == 11) { // revolute jointIdx
        obDouble_.segment(obIdx, 1) = gc_.segment(gvIdx, 1);
        obIdx += 1; gvIdx += 1;
      }
      else {
        obDouble_.segment(obIdx, 3) = gc_.segment(gvIdx, 3);
        obIdx += 3; gvIdx += 3;
      }
  ...
  void computeReward() {
    ...
    float orn_err = 0, orn_reward = 0, vel_err = 0, vel_reward = 0; // , ee_reward = 0, com_reward = 0
    ...
    for (size_t jointIdx=0; jointIdx<12; jointIdx++) {
      if (jointIdx == 3 || jointIdx == 5 || jointIdx == 7 || jointIdx == 10)
        ...
        orn_err += std::pow(acos(quat_err[0]) * 2, 2);
      }
    }
